...
/**
 * Definiciones de las 5 herramientas estándar que todo agente comercial usa.
 * Fuente única de verdad: la usan tanto `HerramientaService.crearHerramientasPorDefecto`
 * (sembrado manual vía endpoint) como `AgenteService.crear` (auto-sembrado al crear un agente).
 *
 * IMPORTANTE: el system prompt de los agentes referencia estos nombres exactos
 * (buscar_producto, calificar_lead, crear_nota, escalar_agente, cambiar_estado).
 * Si un agente no tiene estas filas en la BD, el modelo "escribe" las llamadas como
 * texto (p. ej. `buscar_producto("Geely")`) porque la API nunca recibe las tools.
 */
export const HERRAMIENTAS_DEFAULT: Partial<Herramienta>[] = [
  {
